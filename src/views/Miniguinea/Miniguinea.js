...
//  const activeBanks = banks.filter((bank) => !bank.finished);
  ...
  const reloadNfts = async () => {
    if (account) {
      ...
      setNftStakedTotalSupply(await tombFinance.nftStakedTotalSupply('MiniGuineasStakingNFT'));
    }
  }
  ...
  const selectNftStaked = async (index) => {
    ...
    setReward(await tombFinance.calculateReward(account, nftsStaked[index].tokenId, 'MiniGuineasStakingNFT'));
  }
  ...
  const claim = async () => {
    ...
    setReward(await tombFinance.calculateReward(account, nftsStaked[indexOfSelectedNft].tokenId, 'MiniGuineasStakingNFT'));
  }
